import * as shared from '@altm/shared';
import {
    ConfigFlag, GameFont, IAABB,
    ICookie, ICustomAudioMeta, ICustomBlipMeta, ICustomCheckpointMeta, ICustomColshapeMeta,
    ICustomEntityMeta, ICustomLocalPlayerMeta, ICustomPedMeta, ICustomPlayerMeta,
    ICustomVehicleMeta, ICustomWebViewMeta, IDiscordUser, IHeapStats, IHttpResponse, IInputDevice,
    IMarkerOptions, IProfile, ISyncInfo, IVehicleHandling, IWebViewParams, StatName,
    TextAlign, TextLabelAlignment,
    VehicleIndicatorLights, WebSocketReadyState,
} from '../src/types';
...
export function setCursorPos(pos: shared.IVector2, normalized: boolean): void {
    const val = normalized ? new shared.Vector2(pos).normalize() : pos;
...
export function worldToScreen(xOrValue: number | shared.IVector3, y?: number, z?: number): shared.Vector3 {
    ...
    return new shared.Vector3(screenX, screenY, 0); // Z-Koordinate auf 0 setzen, da es sich um 2D-Koordinaten handelt
}
...
export function screenToWorld(xOrValue: number | shared.IVector2, y?: number): shared.Vector3 {
    ...
    return new shared.Vector3(worldCoord[0], worldCoord[1], worldCoord[2]);
}
...
export function getScreenResolution(): shared.Vector2 {
    return new shared.Vector2(GetScreenResolution());
}
...
export class BaseObject extends shared.BaseObject {
    ...
    public deleteMeta<K extends shared.ExtractStringKeys<shared.ICustomBaseObjectMeta>>(key: string | K): void {
        // TODO implement
    }
    public hasMeta(key: string): boolean;

    public hasMeta<K extends shared.ExtractStringKeys<shared.ICustomBaseObjectMeta>>(key: string | K): boolean {
        // TODO implement
        return false;
    }
    public getMeta<K extends string>(key: Exclude<K, never>): unknown;
    public getMeta<K extends shared.ExtractStringKeys<shared.ICustomBaseObjectMeta>>(key: K): shared.ICustomBaseObjectMeta[K];
    /** @deprecated See {@link ICustomBaseObjectMeta} */
    public getMeta<K extends string>(
        key: K
      ): K extends shared.ExtractStringKeys<shared.ICustomBaseObjectMeta> ? shared.ICustomBaseObjectMeta[K] : unknown {
          // TODO implementieren
          return null as any;
      }
      
    
    public setMeta<K extends string>(key: K, value: shared.InterfaceValueByKey<shared.ICustomBaseObjectMeta, K, unknown, void>): void;
    public setMeta<K extends shared.ExtractStringKeys<shared.ICustomBaseObjectMeta>>(key: K, value: shared.ICustomBaseObjectMeta[K]): void;
    /** @deprecated See {@link ICustomBaseObjectMeta} */
    public setMeta<V extends any, K extends string = string>(key: K, value: shared.InterfaceValueByKey<shared.ICustomBaseObjectMeta, K, V, void>): void;

    public setMeta<K extends string, V>(
        key: K | shared.MetaValues<shared.ICustomBaseObjectMeta>,
        value: K extends shared.ExtractStringKeys<shared.ICustomBaseObjectMeta>
          ? shared.ICustomBaseObjectMeta[K] | shared.InterfaceValueByKey<shared.ICustomBaseObjectMeta, K, V, void>
          : shared.InterfaceValueByKey<shared.ICustomBaseObjectMeta, K, unknown, void>
      ): void {
        // TODO implement
      }
